...
以下设置将根据切割列表中**Thickness**自定义属性的值创建与主装配中提取的**ProductName**自定义属性相对应的子文件夹，并使用下划线符号和钣金零件文档中**PartNo**属性的值命名文件。
...
如果宏报告错误，在某些情况下，可能不会立即明确导致错误的原因，因为错误详细信息被异常处理程序“吞噬”了。为了禁用错误处理并显示导致错误的确切行，请在代码中的所有*On Error GoTo catch_*行之前放置撇号'，如下所示。
...
* 将导出多体钣金零件的每个展开图案。确保使用<\_FeatureName\_>或<$CLPRP:[PropertyName]>区分结果文件
* $PRP和$ASSMPRP值将首先从配置特定属性中提取，如果为空，则从一般文件属性中提取
* 如果指定的属性不存在（对于$CLPRP、$PRP和$ASSMPRP），将使用空字符串作为占位符值
* 宏将处理所有不同的组件（文件路径+配置）
* 宏将自动创建所需的文件夹
* 宏将使用\_替换所有路径无效符号
...
函数`FindCutListFeature`用于查找包含指定`Body2`的切割列表特征。
...
函数`ContainsSwObject`用于检查数组`vArr`中是否包含指定对象`obj`。
