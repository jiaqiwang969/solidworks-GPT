---
caption: 设置顺序组件引用
title: 自动按顺序为SOLIDWORKS组件引用分配编号的宏
description: VBA宏根据种子编号自动递增并按顺序为选定的组件分配组件引用
image: component-reference.png
---
![组件引用](component-reference.png){ width=600 }

这个VBA宏允许自动为SOLIDWORKS装配中选定的组件引用分配数字编号。
...
宏可以配置为在弹出框中指定输入（将**INPUT_SEED**变量的值设置为**True**），或者通过提供种子作为常量来指定输入（**INPUT_SEED**等于**False**，**SEED**等于种子编号）。
