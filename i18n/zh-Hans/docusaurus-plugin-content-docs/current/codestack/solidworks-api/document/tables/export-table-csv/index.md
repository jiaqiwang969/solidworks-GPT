...
但是，该宏保留了逗号、引号或换行符等特殊符号，并根据CSV规范进行了正确的转义：
...
~~~ vb
Const OUT_FILE_PATH_TEMPLATE As String = "<_FileName_>-<_TableName_>.csv" '空字符串以保存在模型文件夹中
Const INCLUDE_HEADER As Boolean = True 'True包含表头，False仅包含数据
Const TABLE_TYPE As Integer = swTableAnnotationType_e.swTableAnnotation_BillOfMaterials  '-1使用所选表格或在swTableAnnotationType_e中定义的表格类型（例如swTableAnnotationType_e.swTableAnnotation_BillOfMaterials导出所有BOM表格）
...
而是使用**-bom**，**-general**，**-revision**，**-cutlist**作为第一个参数来指定要导出的表格类型，并使用可选的输出文件模板作为第二个参数

例如，以下参数将将BOM表格导出为CSV格式，名称为源表格，并保存在D驱动器的**Tables**文件夹中。
...
~~~ vb
#Const ARGS = False 'True使用Toolbar+或Batch+中的参数，而不是常量
