...
caption: 创建可选择的边界框
description: 使用VBA宏基于SOLIDWORKS边界框创建3D边界框草图，并能够选择草图段
...
SOLIDWORKS可以在零件文档中插入3D边界框。然而，该边界框的边缘（段）无法选择和用于建模目的。

这个VBA宏基于SOLIDWORKS 3D边界框创建一个边界框草图。草图中的所有段都可以选择并用于参考或几何创建。

## 注意事项

* 宏将使用现有的3D边界框，如果不存在则创建新的边界框
* 当原始边界框更改（重建后）时，生成的边界框会自动更新
    * 原始边界框必须可见以更新派生边界框
