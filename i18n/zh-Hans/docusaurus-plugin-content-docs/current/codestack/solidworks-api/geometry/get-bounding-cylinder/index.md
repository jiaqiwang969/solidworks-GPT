---
title: 使用SOLIDWORKS API查找体的包围圆柱体
caption: 获取体的包围圆柱体
description: 本示例演示如何使用SOLIDWORKS API查找体的包围圆柱体。
image: cylindrical-bounding-box.png
---
![围绕体创建圆柱形边界框](cylindrical-bounding-box.png){ width=400 }

本示例演示如何使用SOLIDWORKS API查找体的包围圆柱体。

宏要求用户选择输入参数以确定圆柱体的方向。用户可以选择圆形面（在这种情况下，轴将用作参考）或平面特征（在这种情况下，法线将用作参考）。

结果将创建一个新的特征，表示体的圆柱形边界。

> 该宏将为任意方向的体创建最佳拟合的包围圆柱体。体不一定需要与XYZ轴对齐。
...
namespace CodeStack
{
    public partial class SolidWorksMacro
    {
        ...
        private IBody2 GetBodyToProcess(IPartDoc part, object inputObj)
        {
            ...
            throw new NullReferenceException("未找到输入体。请选择圆柱面或使用单个实体零件");
        }
        ...
        private double[] GetDirection(IModelDoc2 model, object inputObj)
        {
            ...
            throw new NullReferenceException("未找到方向。请选择圆柱面或平面");
        }
...
此宏需要引用[Smallest enclosing circle - Library (C#)](https://www.nayuki.io/page/smallest-enclosing-circle)的Project Nayuki。