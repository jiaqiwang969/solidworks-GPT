...
description: Use the SOLIDWORKS API to handle all events related to modifying common or configuration-specific custom properties. Solution to the problem when AddCustomPropertyNotify, DeleteCustomPropertyNotify, and ChangeCustomPropertyNotify events are not triggered.
image: custom-properties-events-console.png
labels: [Custom Properties, Notifications]
---

The SOLIDWORKS API provides notifications to handle modifications to custom properties such as adding, deleting, or changing. These events (AddCustomPropertyNotify, DeleteCustomPropertyNotify, ChangeCustomPropertyNotify) apply to parts, assemblies, and drawings, and support both common and configuration-specific custom properties. However, since SOLIDWORKS 2018, these events are no longer applicable to custom properties modified by users in the user interface and only support custom properties modified through the SOLIDWORKS API.

The code example below provides a solution to this problem and can handle modifications to custom properties regardless of how they are modified.
...
![Outputting property modification information to the console](custom-properties-events-console.png)
