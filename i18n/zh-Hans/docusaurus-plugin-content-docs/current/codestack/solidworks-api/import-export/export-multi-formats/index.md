...
如果此宏用于导出装配体的所有组件，则可能需要将BOM数量包含到文件名中。使用[将SOLIDWORKS装配体中的组件数量写入自定义属性](/docs/codestack/solidworks-api/document/assembly/components/write-quantities/)宏。在导出之前，对装配体运行此宏以创建具有数量值的自定义属性，然后使用**\<Qty\>**占位符将其包含到输出文件名中。
