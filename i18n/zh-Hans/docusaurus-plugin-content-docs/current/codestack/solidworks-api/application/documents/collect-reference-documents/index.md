...
group: Assembly
---
这个VBA宏允许将所有输出文件（如DXF、DWG、PDF等）从所有引用的零件和子装配体文档中的所有级别收集，并复制到指定的文件夹中。

引用的零件和子装配体可以位于任何目录中。主装配体的文件夹或驱动器不需要与其相同。

例如，主装配体**TopAssm1.sldasm**保存在**C:\Assms**文件夹中，并引用位于**D:\Parts\A\Part1.sldprt**和**D:\Parts\B\Part2.sldprt**的两个零件文件。为Part1和Part2创建了DXF和PDF文件，并保存在同一个文件夹中，即**D:\Parts\A\Part1.dxf**、**D:\Parts\A\Part1.pdf**、**D:\Parts\B\Part2.dxf**、**D:\Parts\B\Part2.pdf**。运行此宏后，所有这4个文件将被复制到指定的输出文件夹中。
...
**SEARCH_SUB_FOLDERS**指示宏是否应递归搜索引用的文档。如果将此选项设置为**False**，则只会收集源文件旁边的文件（例如，Part1.dxf必须与Part1.sldprt位于同一个文件夹中）。在某些情况下，输出文件可以放置在子文件夹中（例如，Part1.sldprt的DXFs\Part1.dxf），要收集此类文件，请将**SEARCH_SUB_FOLDERS**设置为**True**。注意，如果任何子文件夹包含具有相同名称的另一个文件，该文件也将被收集（例如，A\B\C\Part1.pdf）。
