...
---
 VBA宏，将随机颜色应用于SOLIDWORKS装配中的所有组件，并可按自定义属性值进行分组
---
...
如果将颜色应用于各个配置（例如**ALL_CONFIGS** = **False**），文档必须具有与配置关联的显示状态，否则颜色不能是配置特定的。
...
```vb
Const PRP_NAME As String = "Type" '按此自定义属性值对组件进行分组，将其设置为空字符串 "" 以不对组件进行分组
