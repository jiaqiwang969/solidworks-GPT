...
description: 使用SOLIDWORKS API，这是一个C# VSTA宏，用于从所选的Boss-Extrude特征中提取选择引用（从实体、结束条件和方向引用）。
image: boss-extrude-property-page.png
labels: [选择，boss-extrude]
---

这个C# VSTA宏使用SOLIDWORKS API从Boss-Extrude特征定义中提取了在From Entity、End Condition和Direction选择框中指定的选择实体的信息。
