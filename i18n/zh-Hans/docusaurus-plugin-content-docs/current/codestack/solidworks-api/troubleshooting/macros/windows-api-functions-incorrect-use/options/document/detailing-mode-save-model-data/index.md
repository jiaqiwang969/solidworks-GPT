---
caption: 切换绘图详细模式保存
title: 使用详细模式保存SOLIDWORKS绘图
description: 使用VBA宏在保存时切换详细模式的开启和关闭
---

在处理大型绘图时，使用详细模式可能会很有益。为了正确使用详细模式，需要将数据保存在文档本身中。

这个过程可能会降低保存性能。

启用或禁用详细模式数据保存的切换选项由文档首选项驱动。

这个宏允许打开或关闭设置并执行文档的保存。
