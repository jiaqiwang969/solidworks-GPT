---
title: 使用SOLIDWORKS API在坐标系之间添加关系
caption: 在坐标系之间添加关系
description: 该宏在两个选定组件的两个坐标系之间添加重合关系
image: sw-mate-coincident.png
labels: [装配体, 零件, 坐标系, 示例, 关系, solidworks api]
redirect-from:
...
使用SOLIDWORKS API在两个选定组件的两个坐标系之间添加重合关系。这些组件必须包含名为*Coordinate System1*的坐标系特征。

![重合关系属性管理器页面](sw-mate-coincident.png){ width=640 }

使用[IAssemblyDoc::AddMate3](https://help.solidworks.com/2018/english/api/sldworksapi/solidworks.interop.sldworks~solidworks.interop.sldworks.iassemblydoc~addmate3.html) SOLIDWORKS API来插入关系特征。
