...
caption: 从外部进程调用内部调用
description: 通过从独立应用程序或宏中调用插件API来实现最大性能
...
当以以下任何一种方式调用插件API时，都会出现完全相同的限制：[通过插件对象](/docs/codestack/solidworks-api/getting-started/inter-process-communication/invoke-add-in-functions/via-add-in-object/)，[通过运行对象表](/docs/codestack/solidworks-api/getting-started/inter-process-communication/invoke-add-in-functions/via-rot/)等。

人们可能错误地认为插件内部的所有SOLIDWORKS API调用都是以内部调用的方式调用的，因为只有一个API函数从独立应用程序中调用。但实际上，SOLIDWORKS插件中的所有SOLIDWORKS API调用都是作为外部调用调用的。这意味着调用插件API将导致与调用独立应用程序相同的性能损失。

然而，有一种方法可以最大化这种性能并通过从外部进程应用程序调用来获得与内部调用相同的结果。
...
使用[SolidWorks.Interop.sldworks.ISldWorks.CommandInProgress](https://help.solidworks.com/2016/English/api/sldworksapi/SolidWorks.Interop.sldworks~SolidWorks.Interop.sldworks.ISldWorks~CommandInProgress.html) SOLIDWORKS API属性可以稍微改善情况，但与基准结果相比，性能仍然下降了10倍以上。
...
回调函数，当内部调用完成时通知独立应用程序。这必须注册为COM对象。
