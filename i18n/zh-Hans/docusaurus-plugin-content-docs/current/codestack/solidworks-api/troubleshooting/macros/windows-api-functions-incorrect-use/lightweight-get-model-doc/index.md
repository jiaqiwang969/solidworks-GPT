...
[IComponent2::GetModelDoc2](https://help.solidworks.com/2012/english/api/sldworksapi/SolidWorks.Interop.sldworks~SolidWorks.Interop.sldworks.IComponent2~GetModelDoc2.html) SOLIDWORKS API方法返回指向[IModelDoc2](https://help.solidworks.com/2012/english/api/sldworksapi/SolidWorks.Interop.sldworks~SolidWorks.Interop.sldworks.IModelDoc2.html)接口的指针。

需要使用该接口来检索模型特定信息（如自定义属性、特征树、注释等）。

对于以轻量级或抑制状态加载的组件，模型文档是不可用的（即指针为*NULL*）。

以下示例演示了如何使用SOLIDWORKS API从组件中获取指向IModelDoc2接口的指针（即使组件处于抑制或轻量级状态）。通过将组件直接加载到内存中，无需解析组件或在其自己的窗口中打开文件即可实现结果。
...
Function GetModelDocFromComponent(comp As SldWorks.Component2) As SldWorks.ModelDoc2
    ...
    If swRefModel Is Nothing Then '组件是轻量级或抑制状态
