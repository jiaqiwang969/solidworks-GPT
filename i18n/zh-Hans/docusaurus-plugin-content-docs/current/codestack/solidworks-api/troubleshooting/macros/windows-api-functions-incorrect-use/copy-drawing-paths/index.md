...
description: 使用SOLIDWORKS API查找活动装配的所有组件的引用图纸路径，并将其复制到剪贴板的VBA宏
...
![在SOLIDWORKS中打开图纸功能](open-component-drawing.png)

这个功能允许逐个查找图纸，但有时需要快速找到该装配的所有组件使用的所有图纸。这可以是自动化软件的一部分。此宏将遍历所有引用并查找所有图纸路径。完成后显示下面的确认消息。
