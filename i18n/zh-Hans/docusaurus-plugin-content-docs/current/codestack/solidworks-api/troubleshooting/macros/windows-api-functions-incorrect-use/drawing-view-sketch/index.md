---
title: 在绘图视图草图中创建草图段落使用SOLIDWORKS API
caption: 在绘图视图中创建草图段落
description: 使用SOLIDWORKS API直接在绘图视图草图区域中创建草图点和草图段落
image: point-in-drawing-view-sketch.png
labels: [绘图,转换,草图]
---
![在绘图视图中创建的点位于中心](point-in-drawing-view-sketch.png){ width=350 }
...
这是一个可以使用[ISketchManager](https://help.solidworks.com/2019/english/api/draftsightapi/Interop.dsAutomation~Interop.dsAutomation.ISketchManager.html)接口绘制草图实体和点的草图。

与[在图纸空间中创建草图段落](/docs/codestack/solidworks-api/document/drawing/sheet-context-sketch/)不同，添加到视图草图中的段落将随视图一起移动，并且在视图的3D旋转时会进行缩放和旋转。

与装配或零件中的草图类似，需要将坐标从模型空间转换为图纸空间，以正确定位段落。

以下示例演示了如何在绘图视图中找到中点（在图纸坐标系中），并使用SOLIDWORKS API使用变换直接在视图中绘制此点。
