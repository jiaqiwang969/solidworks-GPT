...
* <$CLPRP:[属性名]> - 任何切割列表属性的名称，例如\<Thickness\>将替换为切割列表自定义属性*Thickness*的值
* <$PRP:[属性名]> - 钣金零件的任何自定义属性的名称，例如\<PartNo\>将替换为切割列表自定义属性*PartNo*的值
* <$ASSMPRP:[属性名]> - 主装配的任何自定义属性的名称，例如\<ProductId\>将替换为切割列表自定义属性*ProductId*的值
...
以下设置将根据切割列表中**Thickness**自定义属性的值创建与主装配中提取的**ProductName**自定义属性相对应的子文件夹，并使用来自钣金零件文档的**PartNo**属性的值后跟下划线符号和**ProductName**自定义属性的值来命名文件。
...
此宏没有显式变量以在文件名中包含展开图案的数量。但是，可以通过包含自动**QUANTITY**自定义属性的值与**<$CLPRP:QUANTITY>**占位符来提取多体钣金零件的数量值。

为了在装配中包含组件数量，请使用[将SOLIDWORKS装配中的组件数量写入自定义属性](/docs/codestack/solidworks-api/document/assembly/components/write-quantities/)宏。在导出之前运行此宏以创建具有数量值的自定义属性，然后使用**<$CLPRP:Qty>**占位符以将其包含在输出文件名中。

> 注意，此宏不会将多体钣金零件的数量与组件数量相乘

### 展开图案选项

可以通过指定**FLAT_PATTERN_OPTIONS**的值来配置选项。使用**+**组合选项
...
**SKIP_EXISTING_FILES**选项允许指定如果输出文件已存在，宏是否应重新生成输出文件。

将此选项设置为true以跳过导出已存在于目标位置上的文件（.dxf或.dwg）。

~~~ vb
Const SKIP_EXISTING_FILES As Boolean = True
~~~

当处理大型装配并且需要在SOLIDWORKS重新启动后继续执行时，此选项可能很有用。导出展开图案是一项性能较重的操作，因此在处理大型作业时，SOLIDWORKS可能会崩溃或挂起。此选项可帮助在重新启动后继续导出。
...
请提交[错误报告](https://github.com/xarial/codestack/issues/new?labels=bug)并附上此错误和用于重现的模型的快照（如果可能）

## 注意事项

* 如果有轻量级组件，宏将要求解析它们。如果未解析组件，则宏可能会生成错误
* 将导出多体钣金零件的每个展开图案。确保使用<\_FeatureName\_>或<$CLPRP:[PropertyName]>来区分结果文件
* $PRP和$ASSMPRP值将首先从配置特定属性中提取，如果为空，则从一般文件属性中提取
* 如果指定的属性不存在（对于$CLPRP、$PRP和$ASSMPRP），则使用空字符串作为占位符值
* 宏将处理所有不同的组件（文件路径+配置）
* 宏将自动创建所需的文件夹
* 宏将使用\_替换所有路径无效符号
* 宏仅导出分组在切割列表下的唯一实体，并跳过属于已导出切割列表的展开图案
...
try_:
    ...
    swApp.SendMsgToUser2 "操作已完成", swMessageBoxIcon_e.swMbInformation, swMessageBoxBtn_e.swMbOk
    ...
    如果Not IsEmpty（vCutListFeats）Then
        ...
        如果Not IsEmpty（vFlatPatternFeats）Then
            ...
            对于i = 0 To UBound（vFlatPatternFeats）
                ...
                如果TypeOf swFixedEnt Is SldWorks.Face2 Then
                ...
                如果Not swCutListFeat Is Nothing Then

                    Dim isUnique As Boolean

                    如果（Not swProcessedCutListsFeats）= -1 Then
                        isUnique = True
                    ...
                    如果isUnique Then
                        ...
                        如果Not SKIP_EXISTING_FILES Or Not FileExists（outFileName）Then
                            ExportFlatPattern sheetMetalModel，swFlatPatternFeat，outFileName，FLAT_PATTERN_OPTIONS，conf
                        End If
                    End If

                Else
                    Err.Raise vbError，""，"找不到平面图"和swFlatPatternFeat.Name的切割列表
                End If

            Next

        Else
            Err.Raise vbError，""，"找不到平面图特征"
        End If

    Else
        Err.Raise vbError，""，"找不到切割列表项"
    End If

End Sub

函数FileExists（filePath As String）As Boolean
    FileExists = Dir（filePath）<> ""
End Function

函数FindCutListFeature（vCutListFeats As Variant，body As SldWorks.Body2）As SldWorks.Feature

    Dim i As Integer

    对于i = 0 To UBound（vCutListFeats）
        ...
        如果ContainsSwObject（vBodies，body）Then
...
函数ContainsSwObject（vArr As Variant，obj As Object）As Boolean

    如果Not IsEmpty（vArr）Then

        Dim i As Integer

        对于i = 0 To UBound（vArr）

            Dim swObj As Object
            Set swObj = vArr（i）

            如果swApp.IsSame（swObj，obj）= swObjectEquality.swObjectSame Then
...
函数GetFeaturesByType（model As SldWorks.ModelDoc2，typeName As String）As Variant
    ...
    Do While Not swFeat Is Nothing
        ...
        如果typeName = "CutListFolder" And swFeat.GetTypeName2（）= "SolidBodyFolder" Then
            Dim swBodyFolder As SldWorks.BodyFolder
...
子过程ProcessFeature（thisFeat As SldWorks.Feature，featsArr（） As SldWorks.Feature，typeName As String）

    如果thisFeat.GetTypeName2（）= typeName Then

        如果（Not featsArr）= -1 Then
            ReDim featsArr（0）
            Set featsArr（0）= thisFeat
        Else
            Dim i As Integer

            对于i = 0 To UBound（featsArr）
                如果swApp.IsSame（featsArr（i），thisFeat）= swObjectEquality.swObjectSame Then
                    退出子
                End If
            Next
...
子过程ExportFlatPattern（part As SldWorks.PartDoc，flatPattern As SldWorks.Feature，outFilePath As String，opts As SheetMetalOptions_e，conf As String）
...
try_:
    ...
    如果curConf <> conf Then
        如果False = swModel.ShowConfiguration2（conf）Then
            Err.Raise vbError，""，"激活配置失败"
        End If
    End If
    ...
    如果False <> flatPattern.Select2（False，-1）Then
        如果False = part.ExportToDWG2（outFilePath，modelPath，swExportToDWG_e.swExportToDWG_ExportSheetMetal，True，Empty，False，False，opts，Empty）Then
            Err.Raise vbError，""，"导出平面图失败"
        End If
    Else
        Err.Raise vbError，""，"选择平面图失败"
    End If
...
函数GetFullPath（model As SldWorks.ModelDoc2，path As String）

    GetFullPath = path

    如果IsPathRelative（path）Then
...
函数IsPathRelative（path As String）
    IsPathRelative = Mid（path，2，1）<> ":" And Not IsPathUnc（path）
End Function

函数IsPathUnc（path As String）
