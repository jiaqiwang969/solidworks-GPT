...
所有绘图视图都有一个偏移边界。为了得到几何体的实际值，需要从视图尺寸中减去这个边界值。边界值是动态计算的（工作表宽度或高度的2%，取较小值）。这不是一个文档化的值，可能会在未来的SOLIDWORKS版本中更改，这可能会影响到这个宏中的计算。
...
*BASE_VIEWS_ONLY* 变量控制是否对所有视图进行缩放，还是只对基础视图进行缩放（即没有父视图的视图）。如果将此选项设置为 *True*，则会处理所有视图，并且派生视图将与原始源视图断开连接。
...
* 所有宽度最多为100毫米，任何高度的绘图视图将设置为1:1比例
* 所有宽度在100毫米到200毫米之间，高度在50毫米到100毫米之间的绘图视图将设置为1:2比例
    ...
    For i = 0 To UBound(vViews)
        ...
        For j = 0 To UBound(scaleMap)
            ...
            If width >= minWidth And width <= maxWidth And height >= minHeight And height <= maxHeight Then
                ...
                Else
                    Debug.Print "跳过 " & swView.Name & " 视图，因为它不是基础视图"
                End If
