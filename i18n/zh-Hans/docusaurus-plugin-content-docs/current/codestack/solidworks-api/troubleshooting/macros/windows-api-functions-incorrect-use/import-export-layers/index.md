...
description: VBA宏导入和导出SOLIDWORKS图纸中的图层信息（描述、颜色、样式、厚度、可见性和打印属性）
...
默认情况下，文件保存或加载在与原始文件相同的文件夹中，文件名前缀为**_Layers.txt**

## CAD+

这个宏与[Toolbar+](https://cadplus.xarial.com/toolbar/)和[Batch+](https://cadplus.xarial.com/batch/)工具兼容，因此可以将按钮添加到工具栏并分配快捷键以便更容易地访问或批量运行。
...
~~~ vb
#Const ARGS = False '将True更改为使用Toolbar+或Batch+中的参数，而不是常量
