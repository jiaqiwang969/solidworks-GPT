...
caption: 将文件嵌入第三方存储
description: 使用SOLIDWORKS API和模型文档中的XmlSerializers，VB.NET示例演示了如何使用SOLIDWORKS API中的第三方存储（流）来嵌入和提取文件内容
...
* AddFile - 异步方法，用于将嵌入文件数据存储到流中。此方法要求用户选择文件，读取其内容并将其序列化为文件流。
...
* 您可以关闭模型和SOLIDWORKS
* 重新打开模型并单击“LoadFile”。将从模型中反序列化文件数据，并显示“文件另存为”对话框（文件名基于嵌入的文件名）。文件将保存到所选位置
