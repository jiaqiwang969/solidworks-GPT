...
caption: 动画配置
description: 该宏演示了如何使用SOLIDWORKS API从配置中创建动画，以表示模型历史或钣金折叠
...
![钣金折弯动画](motion-study-configuration-animation.png){ width=450 }

可以通过修改宏顶部的常量来更改宏参数（弯曲过渡时间和折叠操作之间的暂停时间）
...
参考[在新配置中禁用特征](/docs/codestack/solidworks-api/document/features-manager/create-feature-configurations/)以获取一个从特征创建配置的宏。
