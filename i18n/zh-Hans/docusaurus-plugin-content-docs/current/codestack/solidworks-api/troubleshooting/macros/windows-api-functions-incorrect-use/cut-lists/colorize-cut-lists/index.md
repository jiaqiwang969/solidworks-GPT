---
layout: sw-tool
title: 用于给SOLIDWORKS钣金和焊接零件切割清单项上色的宏
caption: 上色切割清单
description: SOLIDWORKS VBA宏，根据自定义属性的值为每个切割清单项（钣金和焊接零件）分配唯一的颜色
image: color-cut-list.svg
labels: [切割清单,钣金,焊接,上色]
group: 切割清单
---

这个VBA宏允许根据自定义属性的值为每个切割清单项（钣金或焊接零件）分配一个唯一的颜色。

这个宏最常见的用途是根据剖面尺寸来区分不同类型的焊接零件。

宏将自动为特定的组分配随机颜色。也可以指定用于特定组的固定颜色，而不是随机颜色。
...
例如，要将蓝色（RGB = 0, 0, 255）添加到焊接剖面“50 X 50”，需要添加以下行
