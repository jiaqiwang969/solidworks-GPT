...
group: Drawing
---
![导出为DXF的展开图样](flat-pattern-dxf.png){ width=350 }

这个VBA宏使用SOLIDWORKS API将绘图活动工作表中的所有展开图样视图导出为指定格式（如DXF或DWG）。宏将文件导出到与原始绘图相同的文件夹，并以绘图视图名称命名文件。

如果需要将导出的文件命名为切割清单名称，可以与[使用切割清单名称重命名展开图样视图](/docs/codestack/solidworks-api/document/drawing/rename-sheet-metal-views/)宏结合使用。
