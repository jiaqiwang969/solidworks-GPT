...
description: SolidWorks VBA宏，用于在零件和装配文件中将阴影图像质量设置为粗糙。如果活动文档是装配体，则将选择"应用于所有引用的零件文档"的复选框设置为打开状态。
...
但是，如果您有很多文件并且需要在大型装配体中工作，可以使用宏在打开根装配体之前帮助您使文件变得更轻量化。

此宏设置了上图中显示的两个选项（1）和（2）。选项（2）对零件不可用。

如果您将此宏与Batch+一起使用，将会发挥真正的威力。您可以在不工作于装配体时运行它。
...
' 将文档选项设置为使用粗糙质量
' 如果活动文档是装配体，则将选择"应用于所有引用的零件文档"的复选框设置为打开状态
