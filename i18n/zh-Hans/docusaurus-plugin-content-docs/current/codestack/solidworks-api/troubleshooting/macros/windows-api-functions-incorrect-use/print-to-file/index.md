...
每个注释将会在新的一行中打印。还可以额外包含视图的名称和文件名到输出中。
...
~~~ vb
Const FILE_PATH As String = "" '应写入注释的文本文件的完整路径。如果为空，则使用与原始文件相同的名称，并在前缀处添加_Note.txt
Const PRINT_FILE_NAME As Boolean = True '为True时将文件名输出到文本文件中
Const PRINT_VIEW_NAME As Boolean = True '为True时将绘图视图名称输出到文本文件中
