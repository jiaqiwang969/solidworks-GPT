...
caption: 跟踪ID
description: 本示例演示了在运行合并实体操作时使用面上的跟踪ID的用法
image: select-bodies-faces.png
labels: [跟踪ID, 合并]
---

跟踪ID用于在使用SOLIDWORKS API开发宏和应用程序时，将实体映射（跟踪）到几何操作（如合并、相减、复制、拆分、模式）中。
...
* 选择至少一个面。建议从不同的重叠的实体中选择至少两个面。这样可以演示跟踪ID的好处，因为实体将被合并。
* 运行宏。
...
![通过合并操作创建的单个实体的副本，其中选择了两个面](merged-body.png){ width=250 }
