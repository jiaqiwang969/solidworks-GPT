...
这个VBA宏允许从活动SOLIDWORKS装配或活动零件文档中的所有钣金组件导出到DXF/DWG。

宏允许在输出文件名中指定占位符（原始文件名、特征名、自定义属性、切割列表自定义属性等）与自由文本组合，并支持指定子文件夹。
...
可以是绝对路径或相对路径。如果是后者，则结果将相对于装配目录保存。

必须指定扩展名（.dxf或.dwg）作为命名模板的一部分。
...
* <$CLPRP:[PropertyName]> - 任何切割列表属性的名称，例如\<Thickness\>将被替换为切割列表自定义属性*Thickness*的值
* <$PRP:[PropertyName]> - 钣金零件的任何自定义属性的名称，例如\<PartNo\>将被替换为切割列表自定义属性*PartNo*的值
* <$ASSMPRP:[PropertyName]> - 主装配的任何自定义属性的名称，例如\<ProductId\>将被替换为切割列表自定义属性*ProductId*的值
...
而以下名称将将所有展开图案保存为DWG文件到*D*驱动器上的*Output*文件夹中，其中文件名将从每个相应展开图案的*PartNo*属性中提取。

~~~ vb
Const OUT_NAME_TEMPLATE As String = "D:\Output\<$CLPRP:PartNo>.dwg"
~~~

以下设置将创建与切割列表中的**Thickness**自定义属性值相对应的子文件夹，并使用从主装配中提取的**ProductName**自定义属性值，后跟下划线符号和从钣金零件文档中的**PartNo**属性值的文件名。
...
此宏没有显式变量来在文件名中包含展开图案的数量。但是，可以通过包含自动**QUANTITY**自定义属性的值与**<$CLPRP:QUANTITY>**占位符来提取多体钣金零件的数量值。
...
在处理大型装配时，此选项可能很有用，并且需要在SOLIDWORKS重新启动后继续执行。导出展开图案是一个性能消耗很大的操作，因此在处理大型作业时，SOLIDWORKS可能会崩溃或挂起。此选项可以帮助在重新启动后继续导出。
...
* 宏将自动创建所需的文件夹
* 宏将使用\_替换所有路径无效的符号
* 宏仅导出切割列表下的唯一体，并跳过属于已导出切割列表的展开图案
