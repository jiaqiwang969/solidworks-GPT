...
layout: sw-tool
caption: 添加显示数据标记
title: 将显示数据标记添加到主SOLIDWORKS装配所使用的配置的宏
description: 用于在大型装配的大型设计审查模式中打开或在eDrawings中支持配置的VBA宏
image: display-data-mark.svg
group: Assembly
---
这个VBA宏对于在大型设计审查模式下使用装配件的用户或需要在eDrawings中支持配置的用户非常有用。

默认情况下，只有活动配置被保留以在大型设计审查模式下使用，而装配的其他配置无法被激活：
...
该宏的另一版本仅处理活动零件或装配的配置，并添加显示数据标记
