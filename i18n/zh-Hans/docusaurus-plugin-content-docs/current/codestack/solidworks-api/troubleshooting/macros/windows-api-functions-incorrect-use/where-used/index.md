...
caption: 查找使用情况
description: 使用SOLIDWORKS API查找活动装配体中使用所选组件的装配体的VBA宏
image: where-used-form.png
labels: [使用情况,父级,组件]
group: 装配体
---
这个VBA宏使用SOLIDWORKS API在活动装配体中查找所选组件实例的所有父组件（使用情况），并显示列表供查看。

![带有父组件列表的使用情况表单](where-used-form.png){ width=350 }

在表单中可以选择所有引用，并在特征管理器树中突出显示相应的组件。
