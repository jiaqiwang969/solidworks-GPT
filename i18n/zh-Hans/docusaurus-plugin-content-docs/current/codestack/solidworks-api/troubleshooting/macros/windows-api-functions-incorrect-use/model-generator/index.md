...
Public Class ModelGenerator
    ...
    Public Shared Sub GenerateModelFromTemplate(templatePath as String, outFilePath As String, width As String, length As String, height As String)
        ...
        Console.WriteLine("打开模板模型: " + templatePath)
        ...
        If model IsNot Nothing Then
            ...
            Try
                ...
                For Each paramData As KeyValuePair(Of String, Double) In parameters
                    ...
                    If param IsNot Nothing Then
                        ...
                        Else
                            Throw New Exception(String.Format("将参数{0}设置为{1}失败", paramName, paramValue))
                        End If
                    Else
                ...
                Console.WriteLine("将模型保存到" + outFilePath)
                ...
                If err <> 0  Then
                    Throw New Exception(String.Format("将文档保存到{0}失败。错误代码: {1}", outFilePath, err))
                End If
            ...
            End Try
        Else
            Console.WriteLine("打开模板模型失败 " + templatePath)
        End If
...
下载[模板模型](template.SLDPRT)并保存到与上述两个脚本相同的文件夹中。
...
![控制台中显示进程进展和模型生成结果的消息](console-output.png){ width=450 }

模板文件不会被修改，生成的模型将带有更新后的参数保存。
