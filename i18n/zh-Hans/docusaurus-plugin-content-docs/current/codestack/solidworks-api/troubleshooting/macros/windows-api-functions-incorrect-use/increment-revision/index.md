---
layout: sw-tool
title: 通过SOLIDWORKS API增加注释中的数字值的宏特征
caption: 增加注释中的数字值
description: 该宏通过匹配正则表达式（例如增加修订版本）使用SOLIDWORKS API增加注释的数字值
image: drawing-revision-incremented.png
labels: [note, revision, increment, regular expression, regex, tag]
group: Drawing
---
![在标题块中增加修订号](drawing-revision-incremented.png){ width=300 }

该宏使用SOLIDWORKS API增加注释的数字值。如果需要增加注释的修订版本而无需手动选择和更改注释，则这将非常有用。该宏还可以在批处理软件中使用。

![通过运行宏按钮来增加标题块中的修订号](increment-revision-macro.gif)
...
* 需要在注释中添加文本标签以增加其值。请按照[为选定的注释添加标签](/docs/codestack/solidworks-api/document/notes/tag-selected-note)示例中的说明添加标签到注释中。
* 默认情况下，数字值增加1，但可以通过修改*IncrementNoteValue*函数的*increment*参数的值来更改。
