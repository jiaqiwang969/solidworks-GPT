...
此宏演示了如何使用演示变换通过[SOLIDWORKS API中的IComponent2::PresentationTransform](https://help.solidworks.com/2012/english/api/sldworksapi/solidworks.interop.sldworks~solidworks.interop.sldworks.icomponent2~presentationtransform.html)方法运行组件围绕组件的Y轴的平滑旋转动画。

这允许仅为了视觉效果而移动组件，而不改变几何形状。组件将被移动，无论它在空间中是否完全定义（通过配合件或固定约束）。并且配合件仍然保留。
