...
caption: 导出草图坐标
description: VBA宏，用于将选定草图中的草图点坐标导出到CSV文件中，并能够将坐标转换为用户单位和模型空间
image: export-coordinates.svg
labels: [sketch,export,points,coordinates,csv]
group: Sketch
---
![选定草图中的草图点](sketch-points.png){ width=500 }

这个VBA宏允许将选定草图中的所有草图点的坐标导出到CSV文件中。
...
Const CONVERT_TO_MODEL_SPACE As Boolean = True '对于2D草图，True表示将坐标导出到草图空间，False表示将坐标转换为模型空间
