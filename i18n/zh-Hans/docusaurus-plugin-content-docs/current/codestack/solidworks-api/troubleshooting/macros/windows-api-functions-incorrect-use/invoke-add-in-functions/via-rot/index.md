...
sidebar_position: 2
---
本文提供了如何通过运行对象表（ROT）从独立应用程序或脚本调用SOLIDWORKS插件函数的详细的逐步说明。这可以被视为在插件本身中启用应用程序编程接口（API）。

这种方法允许通过从进程ID提取它来连接到API对象。这种方法的主要好处之一是能够在不直接使用任何SOLIDWORKS API命令甚至添加SOLIDWORKS类型库或互操作的情况下控制插件API。

在这个例子中，SOLIDWORKS插件允许计算所选实体的面数。它提供了一个菜单供用户点击。
...
这是一个帮助COM对象，它将简化从进程中按其ID检索API对象的访问。它负责从进程中检索API对象的实例。
...
这是一个SOLIDWORKS插件。在这个例子中，它使用[SwEx.AddIn Framework](/docs/codestack/labs/solidworks/swex/add-in/)进行开发，但是任何其他框架或SDK都将得到支持。

### GeometryHelperService.cs

插件内的辅助类，用于调用SOLIDWORKS API来计算所选实体上的面数，基于最小面积。
...
* 通过实现代理API对象。这种方法可能被认为更有益和安全，因为它不会在其结构中公开任何内部对象。所有的请求都在代理类之外处理和处理。

### GeometryHelperApiObjectProxy.cs

代理对象不包含对插件中任何对象的引用。相反，它将生成请求事件，由插件处理和处理。
...
宏通过其Prog ID创建工厂对象的实例，并通过进程ID检索API对象的实例。
...
如下所示，只需要添加对API对象dll的引用，该dll包含接口的定义。不需要添加任何其他引用（包括插件dll或任何SOLIDWORKS互操作）。
