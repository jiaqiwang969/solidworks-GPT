...
description: 使用VBA宏根据预定义模板（例如自定义属性值）重命名SOLIDWORKS切割列表体文件夹（钣金或焊接）中的体
image: renamed-cut-list-bodies.png
group: 切割列表
---
![在特征管理器中重命名的钣金和焊接体](renamed-cut-list-bodies.png)

这个VBA宏允许根据预定义的命名模板重命名所有属于切割列表文件夹（例如钣金或焊接）的体。可以使用自由文本结合自定义属性占位符，以便在名称中使用自定义属性。
...
* 在运行后，宏可能需要模型重建以刷新特征的名称
* 如果一个切割列表文件夹中有多个体，则将使用索引来区分名称，例如-1，-2，-3
* 在重命名之前，宏将执行切割列表更新
