...
caption: 使用图纸进行独立复制
description: VBA宏允许复制所选组件的独立副本，更新装配体中的引用并复制相关的图纸
image: make-independent-drawing.svg
labels: [独立复制,图纸,组件]
group: 装配体
---
此VBA宏模仿了SOLIDWORKS的**独立复制**功能，但还会额外复制并重命名与复制的零件或装配体组件相关联的文件。

![独立复制菜单命令](make-independent-menu.png)

此宏可以处理单个组件或多个选定的组件，但所有组件都必须对应于同一个文件。
