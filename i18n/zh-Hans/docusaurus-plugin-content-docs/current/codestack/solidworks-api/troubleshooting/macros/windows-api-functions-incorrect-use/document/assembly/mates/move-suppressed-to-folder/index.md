---
layout: sw-tool
title: 使用SOLIDWORKS API将禁用的约束移动到特征文件夹的宏
caption: 将禁用的约束移动到文件夹
description: 使用SOLIDWORKS API将装配中的所有禁用的约束移动到指定的特征管理器文件夹的VBA宏
image: move-mates-to-folder.png
labels: [约束,禁用,移动,文件夹]
group: 装配
---
![已移动到文件夹的禁用约束](suppressed-solidworks-mates.png){ width=250 }

这个VBA宏允许使用SOLIDWORKS API将所有禁用的约束移动到指定的特征管理器文件夹中。如果文件夹不存在，宏将创建文件夹，如果文件夹已存在，则将移动到已存在的文件夹中。

如果文件夹中存在未禁用的约束，宏也会将它们移动到文件夹中。
...
~~~ vb
Const FOLDER_NAME As String = "禁用的约束"
