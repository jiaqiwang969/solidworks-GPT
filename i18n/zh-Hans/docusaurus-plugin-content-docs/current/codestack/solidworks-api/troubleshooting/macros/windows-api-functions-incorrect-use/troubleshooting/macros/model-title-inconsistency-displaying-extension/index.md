...
![运行宏时出现的“运行时错误 '5'：无效的过程调用或参数”错误](invalid-procedure-or-call-error.png){ width=640 height=211 }
...
![Windows资源管理器中的“隐藏已知文件类型的扩展名”选项](hide-extensions-for-known-file-types.png){ width=277 height=320 }

* 对于新创建的文件（即从未保存过的文件），不会显示扩展名。
* 对于绘图，标题是名称和活动图纸的组合。绘图不显示扩展名。
